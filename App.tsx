...
import { PASTORAL_LESSON_6 } from './data/lessonContent';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (isPlaying) {
      interval = window.setInterval(() => {
        if (currentSlideIndex < PASTORAL_LESSON_6.totalSlides - 1) {
          nextSlide();
        } else {
          toggleAutoplay();
        }
  ...
  const currentSlide = PASTORAL_LESSON_6.slides[currentSlideIndex];
  const total = PASTORAL_LESSON_6.totalSlides;
          ...
          <div className="relative w-full max-w-4xl bg-[#1a1a1a] rounded-[3rem] border border-white/10 shadow-2xl overflow-hidden flex flex-col">
            <div className="p-10 border-b border-white/5 flex items-center justify-between bg-gradient-to-r from-red-600/10 to-transparent">
              <div>
                ...
                <p className="text-xs font-bold text-red-500 uppercase tracking-widest mt-1">Plataforma Educativa Pastoral</p>
              </div>
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex gap-4 p-4 hover:bg-white/5 rounded-2xl transition-all">
                    ...
                    <div>
                      <p className="text-sm font-black uppercase tracking-wide">Tarjetas Interactivas</p>
                      <p className="text-xs opacity-50 font-medium leading-relaxed">Gira y expande las tarjetas para profundizar en cada requisito.</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      `}>
        <div className="h-full flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-1 custom-scrollbar">
            {PASTORAL_LESSON_6.slides.map((s, idx) => (
      ...
      <main className="flex-1 flex flex-col h-full relative bg-[#111111]">
        <header className="h-16 px-6 lg:px-10 flex items-center justify-between border-b border-white/5 bg-[#1a1a1a] z-50 shrink-0">
           <div className="flex items-center gap-6">
              ...
              <h1 className="hidden sm:block text-xs font-black uppercase tracking-widest text-white/80">{PASTORAL_LESSON_6.subtitle}</h1>
           </div>
